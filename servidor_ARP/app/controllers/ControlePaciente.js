...
class ControlePaciente{
    ...
    async paciente(req, res) {
        try {
            ...
            if (paciente) {
                ...
                res.status(404).json({ error: 'Paciente não foi encontrado' });
            }
        } catch (err) {
    ...
    async atualizar(req, res) {
        try {
            ...
            if (updatedRows) {
                res.status(200).json({ message: 'Dados do paciente foram atualizados com sucesso' });
            } else {
                res.status(404).json({ error: 'Paciente não foi encontrado' });
            }
        } catch (err) {
    ...
    async deletar(req, res) {
        try {
            ...
            if (deletedRows) {
                res.status(200).json({ message: 'Paciente foi deletado com sucesso' });
            } else {
                res.status(404).json({ error: 'Paciente não foi encontrado' });
            }
        } catch (err) {
...
module.exports = ControlePaciente;